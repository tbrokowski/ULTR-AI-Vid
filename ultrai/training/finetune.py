#!/usr/bin/env python3
"""
Public entrypoint for model fine-tuning.

There are two supported modes:
1. `--target-dataset sa`
   Runs the cross-domain target-domain pipeline for SA:
   - build the SA 60/40 split
   - optional zero-shot evaluation
   - fine-tune on SA subsets
   - optional source-domain re-evaluation
2. `--target-dataset benin`
   Runs a standard warm-start fine-tuning pass on Benin by forwarding the
   source checkpoint into the supervised core trainer.
"""
...
from ultrai.training import ablation_engine, train as train_entry


def _use_dataset_adapter(dataset_name: str):
    """Tell the ablation engine which dataset adapter to use for the next trainer."""
...
def load_checkpoint_state_dict(checkpoint_path: str, device: torch.device) -> Dict:
    """Load a source checkpoint and return the model state dict."""
    logger.info(f"Loading source checkpoint from {checkpoint_path}")
...
def save_comprehensive_predictions(
    ...
    Args:
        ...
        prefix: Prefix for filenames (e.g., 'source_zero_shot', 'finetuned')
...
def evaluate_zero_shot_on_target(
    ...
    """
    Evaluate a source checkpoint zero-shot on the SA target test set.
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info("Step 6: Evaluating Source Checkpoint (Zero-shot)")
    logger.info(f"{'='*60}")
    logger.info("Evaluating source checkpoint (zero-shot) on SA test set...")

    _use_dataset_adapter("sa")
    ...
    # Create split CSV
    test_split_csv = os.path.join(output_dir, 'splits', 'source_zero_shot_split.csv')
    create_split_csv(
        train_ids=[],
        val_ids=[],
        test_ids=test_ids,
    ...
    from ultrai.training.ablation_engine import setup_distributed, cleanup_distributed, AblationTrainer, Config
    ...
    try:
        ...
        eval_config = Config()
        eval_config.load_from_yaml(config.__dict__.get('_yaml_path', 'configs/cscs/finetune.yaml'))
        eval_config.split_csv = test_split_csv
        eval_config.experiment_name = "source_zero_shot_eval"
        eval_config.experiment_dir = os.path.join(output_dir, "source_zero_shot")
        eval_config.log_dir = os.path.join(config.log_dir, "source_zero_shot")
        ...
        # Initialize trainer with the active SA adapter.
        logger.info("  Creating trainer with SA dataset adapter...")
        ...
        source_state_dict = load_checkpoint_state_dict(source_checkpoint_path, trainer.device)
        ...
        logger.info(f"  Loaded {len(filtered_state_dict)}/{len(source_state_dict)} parameters")
        ...
        # Save results
        results_path = os.path.join(eval_config.experiment_dir, 'source_zero_shot_results.json')
...
def run_finetuning_experiment(
    ...
    try:
        ...
        for subset_name, train_subset_df in train_subsets.items():
            ...
            subset_config.experiment_name = f"target_finetune_{subset_name}"
            ...
            subset_config.log_dir = os.path.join(config.log_dir, f"finetune_{subset_name}")  # Local logs
...
def evaluate_on_source_test(
    ...
    _use_dataset_adapter(source_dataset)
    ...
    try:
        ...
        # Initialize trainer with the source dataset adapter.
        trainer = AblationTrainer(eval_config, rank, world_size, local_rank)
        
        # Load fine-tuned checkpoint
        finetuned_state_dict = load_checkpoint_state_dict(finetuned_checkpoint_path, trainer.device)
        ...
        if source_dataset == "benin" and os.path.exists(hmv_mil_path):
            ...
            if 'tb_prob' in hmv_mil_df.columns and 'tb_label' in hmv_mil_df.columns:
                ...
                logger.info(f"  Target-finetuned - AUROC: {auroc:.4f}, AUPRC: {auprc:.4f}")
...
def generate_pathology_auroc_plots(
    ...
    # Switch back to the SA target adapter for pathology evaluation.
    _use_dataset_adapter("sa")
    ...
    try:
        ...
        # Load checkpoint
        checkpoint_state_dict = load_checkpoint_state_dict(finetuned_checkpoint_path, trainer.device)
...
def _run_benin_target_finetuning(args):
    forwarded_args = [
        "--dataset", "benin",
        "--config", args.config,
        "--model-weights", args.source_checkpoint,
    ]

    if args.clip_unfreeze_last_n_layers is not None:
        forwarded_args.extend(["--clip-unfreeze-last-n-layers", str(args.clip_unfreeze_last_n_layers)])
    if args.freeze_backbone is True:
        forwarded_args.append("--freeze-backbone")
    elif args.freeze_backbone is False:
        forwarded_args.append("--no-freeze-backbone")

    logger.info("%s", "=" * 80)
    logger.info("Benin Target Fine-Tuning")
    logger.info("%s", "=" * 80)
    logger.info("Delegating warm-start Benin target fine-tuning to the supervised core trainer")
    logger.info("  config:             %s", args.config)
    logger.info("  source_checkpoint:  %s", args.source_checkpoint)
    return train_entry.main(forwarded_args)


def main(argv=None):
    """Public fine-tuning entrypoint."""
    parser = argparse.ArgumentParser(description="Dataset-aware fine-tuning pipeline")
    parser.add_argument("--config", type=str, default="configs/cscs/finetune.yaml",
                        help="Path to the finetuning stage config YAML")
    parser.add_argument("--target-dataset", choices=["sa", "benin"], default="sa",
                        help="Dataset to fine-tune on. Default: sa.")
    parser.add_argument("--source-dataset", choices=["benin", "sa"], default="benin",
                        help="Dataset the source checkpoint came from. Default: benin.")
    parser.add_argument("--output-dir", type=str, default="./finetuning_results",
                        help="Output directory for results, split CSVs, and plots")
    parser.add_argument("--checkpoint-dir", type=str, default="./checkpoints/finetune",
                        help="Directory for model checkpoints written by the finetuning stage")
    parser.add_argument("--log-dir", type=str, default="./logs",
                        help="Directory for trainer-side logs written during fine-tuning")
    parser.add_argument("--source-checkpoint", "--benin-checkpoint", dest="source_checkpoint", type=str,
                        help="Path to the source checkpoint used for warm-starting fine-tuning")
    parser.add_argument("--source-config", "--benin-config", dest="source_config", type=str,
                        help="Optional source-domain config used for source re-evaluation after fine-tuning")
    parser.add_argument("--split-csv", type=str, default=None,
                        help="Explicit split CSV for Benin-target fine-tuning")
    parser.add_argument("--clip-unfreeze-last-n-layers", type=int,
                        help="Override how many CLIP encoder blocks are fine-tuned")
    parser.add_argument("--freeze-backbone", action="store_true",
                        help="Keep the CLIP encoder frozen during fine-tuning")
    parser.add_argument("--no-freeze-backbone", dest="freeze_backbone", action="store_false",
                        help="Allow CLIP encoder fine-tuning during fine-tuning")
    parser.add_argument("--skip-zero-shot", action="store_true",
                        help="Skip zero-shot evaluation for the SA target pipeline")
    ...
    logger.info("Output directory for plots/results (absolute): %s", os.path.abspath(args.output_dir))
    logger.info("Checkpoint directory for model weights (absolute): %s", os.path.abspath(args.checkpoint_dir))
    logger.info("Local log directory (absolute): %s", os.path.abspath(args.log_dir))
    logger.info("Checkpoints will be saved to: %s/[subset_name]/checkpoint_best.pth", args.checkpoint_dir)
    ...
    logger.info("Successfully loaded %d patients with LUS data", len(merged_df))
    logger.info("  TB+: %d patients", merged_df["TB_Label"].sum())
    logger.info("  TB-: %d patients", (merged_df["TB_Label"] == 0).sum())
    ...
    for subset_name, subset_df in train_subsets.items():
        ...
        logger.info("  %s: %d train, %d val, %d test", subset_name, len(train_ids), len(val_ids), len(test_ids_list))

    test_ids_list = test_df["patient_id"].tolist()

    if not args.skip_zero_shot:
        logger.info("\n%s", "=" * 60)
        logger.info("Step 6: Evaluating Source Model (Zero-Shot)")
        logger.info("%s", "=" * 60)
        try:
            zero_shot_results = evaluate_zero_shot_on_target(
                source_checkpoint_path=args.source_checkpoint,
                test_ids=test_ids_list,
                config=config,
                output_dir=args.output_dir,
            )
            logger.info("Zero-shot evaluation complete: AUROC=%.4f", zero_shot_results.get("auroc", float("nan")))
        except Exception as e:
            logger.error("Zero-shot evaluation failed: %s", e)
    else:
        logger.info("Skipping zero-shot evaluation")
    ...
    if not args.skip_finetuning:
        ...
        try:
            ...
            logger.info("  Best AUROC: %.4f", finetuning_results.get("best_auroc", float("nan")))
        except Exception as e:
    ...
    if not args.skip_source_eval and best_checkpoint and args.source_config:
        logger.info("\n%s", "=" * 60)
        ...
        if args.skip_source_eval:
            ...
            logger.warning("No best checkpoint available for source-domain evaluation")
        elif not args.source_config:
            logger.warning("No source config provided for source-domain evaluation")
    ...
    if not args.skip_pathology and best_checkpoint:
        ...
        try:
            ...
            logger.info("Pathology plots generated")
            for path_name, metrics in pathology_results.items():
                logger.info("  %s: AUROC=%.3f", path_name, metrics.get("auroc", float("nan")))
        except Exception as e:
            logger.error("Pathology plot generation failed: %s", e)
    else:
        if args.skip_pathology:
            logger.info("Skipping pathology plots")
        elif not best_checkpoint:
            logger.warning("No best checkpoint available for pathology evaluation")

    logger.info("\n%s", "=" * 80)
    logger.info("SA Fine-Tuning Pipeline Complete")
    logger.info("%s", "=" * 80)
    logger.info("Results saved to: %s", args.output_dir)
    logger.info("Total patients: %d", len(merged_df))
    logger.info("  Training pool: %d", len(train_df))
    logger.info("  Test set: %d", len(test_df))
