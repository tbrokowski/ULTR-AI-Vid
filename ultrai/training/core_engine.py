...
from ultrai.training.config import parse_args_and_load_config
...
class TBTrainer:
    ...
    def _setup_data(self):
        self.data_module = _DATASET_ADAPTER.LungUltrasoundDataModule(
    ...
    def _reset_for_epoch(self):
        ...
        if self.selection_strategy == 'RL' and hasattr(self, 'frame_selector_optimizer'):
    ...
    def _compute_site_specific_rewards(self, outputs, targets, batch_idx):
            ...
            with torch.no_grad():
                ...
                if 'TB Label' in task_logits:
                    ...
                    if tb_label >= 0:
                        ...
                        base_reward = 2.0 * tb_correct - 1.0
                        confidence = torch.abs(tb_prob - 0.5)
                        confidence_multiplier = 1.0 + confidence

                        patient_reward = base_reward * confidence_multiplier
                        patient_reward = patient_reward.item()
                    ...
                    for p in range(self.config.num_pathologies):
                        if site_pathology_labels[p] >= 0:
                            with torch.no_grad():
                                ...
                                path_reward = 2.0 * path_correct - 1.0
                                path_reward *= 0.5
                                path_reward = path_reward.item()
        ...
        if batch_idx % 20 == 0 and reward_components['combined_rewards']:
            ...
            logger.info(
                f"  Patient: {np.mean(patient_rewards):.3f}±{np.std(patient_rewards):.3f} "
            ...
            logger.info(
                f"  Combined: {np.mean(combined_rewards):.3f}±{np.std(combined_rewards):.3f} "
            ...
            if getattr(self.config, 'adaptive_rl_adjustments', True):
                ...
                if abs(reward_trend) < 0.005 and len(self.rl_training_history['rewards']) > 50:
                    if hasattr(self.model.frame_selector, 'temperature'):
                        ...
                        logger.info(
                            f"Increased temperature to {self.model.frame_selector.temperature:.4f} for exploration"
                        )
        ...
        try:
            ...
            for i, (action_data, original_reward) in enumerate(action_reward_pairs):
                ...
                if state_value is not None and state_value.requires_grad:
                    state_value_scalar = state_value.squeeze().float()
                    reward_scalar = reward_tensor.squeeze().float()
                ...
                policy_loss = -log_prob.float() * advantage.float()
            ...
            total_value_loss = (
                torch.stack(value_losses).mean().float()
                if value_losses
                else torch.tensor(0.0, dtype=torch.float32, device=total_policy_loss.device)
            )
            ...
            if batch_idx % 20 == 0:
                ...
                log_msg = (
                    f'RL Update - Loss: {rl_loss_value:.4f}, '
                ...
                log_msg += " [STEP]" if is_accumulation_end else " [ACCUM]"
                logger.info(log_msg)
    ...
    def train_epoch(self, epoch):
                                    ...
                                    if valid_mask.any():
                                        ...
                                        if (batch_idx + 1) % accumulation_steps == 0 or (batch_idx + 1 == len(self.train_loader)):
                                            ...
                                            torch.nn.utils.clip_grad_norm_(
                                                [p for name, p in self.model.named_parameters() if f'pathology_modules.{path_idx}' in name and p.requires_grad],
                                                max_norm=1.0
                                            )
        ...
        logger.info("Train TB metrics:")
        tb_metrics = {k.replace('TB Label_', ''): v for k, v in all_metrics.items() if k.startswith('TB Label_')}
        if tb_metrics:
            logger.info("  TB Label: " + " | ".join([f"{k}: {v:.4f}" for k, v in tb_metrics.items()]))

        if self.use_pathology_loss:
            path_metrics = {k: v for k, v in all_metrics.items() if '/' in k}
            if path_metrics:
                logger.info("Train Pathology metrics:")
                pathology_names = getattr(
                    self.config,
                    'pathology_classes',
                    ['A-line', 'Large consolidations', 'Pleural Effusion', 'Other Pathology']
                )
                for name in pathology_names:
                    name_metrics = {k.split('/')[-1]: v for k, v in path_metrics.items() if k.startswith(f'{name}/')}
                    if name_metrics:
    ...
    def validate(self, epoch, loader=None, split_name="val"):
            ...
            for batch in progress_bar:
                try:
                    ...
                    if 'out of memory' in str(e):
                        logger.warning("WARNING: Out of memory during validation, skipping batch")
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                        continue
                    else:
        ...
        if all_tb_targets:
            ...
            try:
                cm = confusion_matrix(tb_targets, tb_predictions, labels=[0, 1])
    ...
    def train(self, resume_from_checkpoint=None):
        ...
        for epoch in range(start_epoch, self.config.num_epochs):
            ...
            if self.config.eval_metric_goal == 'max':
                if current_metric > self.best_metric:
                    ...
                    logger.info(
                        f"No improvement. Best {eval_metric_key} is still: {self.best_metric:.4f} "
        ...
        logger.info(
            f"Training completed. Best model from epoch {self.best_epoch+1} "
            f"with {eval_metric_key}: {self.best_metric:.4f}"
        )
    ...
    def _evaluate_best_model(self):
        ...
        pathology_class_names = (
            getattr(self.config, 'pathology_classes', [f"pathology_{i}" for i in range(self.config.num_pathologies)])
            if self.use_pathology_loss else []
        )

        for split_name, loader, file_prefix in splits:
            logger.info(f"Evaluating on {split_name} set...")

            detailed_results, tb_metrics, pathology_metrics, pathology_curve_data = self._run_detailed_tb_evaluation(
                loader, split_name
            )
            ...
            tb_metrics_str = " | ".join(
                [f"{k}: {v:.4f}" for k, v in tb_metrics.items() if k != 'loss' and isinstance(v, (int, float))]
            )
            ...
            if self.use_pathology_loss:
                for pathology_name, metrics in pathology_metrics.items():
                    ...
                    curve_data = pathology_curve_data.get(pathology_name)
                    if curve_data is not None and 'auc' in metrics and not np.isnan(metrics['auc']):
                        self._plot_roc_curve(
                            curve_data['targets'],
                            curve_data['probabilities'],
    ...
    def _create_consolidated_pathology_summary(self, all_pathology_metrics, pathology_class_names, output_file):
        try:
            ...
            consolidated_columns = ['Split', 'Pathology'] + metrics_to_include
            consolidated_df = pd.DataFrame(consolidated_data, columns=consolidated_columns)
            consolidated_df.to_csv(output_file, index=False)

            if consolidated_df.empty:
                logger.warning(f"No pathology metrics available to summarize. Saved empty file to {output_file}")
                return
            ...
            for metric in metrics_to_include:
                if metric not in consolidated_df.columns:
                    logger.info(f"Skipping pivot table for {metric} - metric column not available")
                    continue

                metric_subset = consolidated_df[['Pathology', 'Split', metric]].dropna(subset=[metric])
                if metric_subset.empty:
                    logger.info(f"Skipping pivot table for {metric} - no valid values available")
                    continue

                metric_pivot = metric_subset.pivot_table(
                    index='Pathology',
                    columns='Split',
                    values=metric
                )
                metric_output = pivot_file.replace('.csv', f'_{metric}.csv')
                metric_pivot.to_csv(metric_output)
                logger.info(f"Pivot table for {metric} saved to {metric_output}")
    ...
    def _run_detailed_tb_evaluation(self, loader, split_name):
        ...
        Returns:
            tuple: (detailed_results_df, tb_metrics_dict, pathology_metrics_dict, pathology_curve_data_dict)
        """
        self.model.eval()
        all_patient_ids = []
            ...
            for batch in progress_bar:
                try:
                    ...
                    if first_batch:
                        ...
                        if site_findings.dim() >= 3:
                            pathology_dim = site_findings.shape[-1]
                        elif site_findings.dim() == 2:
                            pathology_dim = site_findings.shape[1]
                        else:
                            pathology_dim = 1
                    ...
                    if self.use_pathology_loss and 'pathology_scores' in outputs:
                        ...
                        if pathology_logits.dim() == 3:
                            _, _, num_pathologies = pathology_logits.shape
                            pathology_targets = site_findings.reshape(-1, num_pathologies)
                            pathology_logits = pathology_logits.reshape(-1, num_pathologies)
                            pathology_probs = pathology_probs.reshape(-1, num_pathologies)
                            pathology_preds = pathology_preds.reshape(-1, num_pathologies)
                        elif pathology_logits.dim() == 2:
                            pathology_targets = site_findings
                        else:
                            pathology_targets = site_findings.reshape(-1, 1)
                            pathology_logits = pathology_logits.reshape(-1, 1)
                            pathology_probs = pathology_probs.reshape(-1, 1)
                            pathology_preds = pathology_preds.reshape(-1, 1)

                        all_pathology_targets_np.append(pathology_targets.detach().cpu().numpy())
                    ...
                    if 'out of memory' in str(e):
                        logger.warning("WARNING: Out of memory during evaluation, skipping batch")
                        if torch.cuda.is_available():
                            torch.cuda.empty_cache()
                        continue
                    else:
        ...
        pathology_metrics = {}
        pathology_curve_data = {}

        if self.use_pathology_loss and all_pathology_targets_np and pathology_dim is not None:
            try:
                pathology_targets = np.concatenate(all_pathology_targets_np, axis=0)
                pathology_logits = np.concatenate(all_pathology_logits_np, axis=0)
                pathology_probs = np.concatenate(all_pathology_probs_np, axis=0)
                pathology_preds = np.concatenate(all_pathology_preds_np, axis=0)

                if pathology_targets.ndim == 1:
                    pathology_targets = pathology_targets.reshape(-1, 1)
                    pathology_logits = pathology_logits.reshape(-1, 1)
                    pathology_probs = pathology_probs.reshape(-1, 1)
                    pathology_preds = pathology_preds.reshape(-1, 1)

                num_pathology_columns = pathology_targets.shape[1]

                pathology_class_names = getattr(self.config, 'pathology_classes', [
                    f"pathology_{i}" for i in range(num_pathology_columns)
                ])

                for i, class_name in enumerate(pathology_class_names[:num_pathology_columns]):
                    ...
                    if valid_mask.sum() == 0:
                        logger.info(f"Skipping {class_name} metrics - no valid labels present")
                        continue

                    class_targets_valid = class_targets[valid_mask]
                    class_preds_valid = class_preds[valid_mask]
                    class_probs_valid = class_probs[valid_mask]

                    pathology_metrics[class_name] = self._calculate_binary_metrics(
                        class_targets_valid,
                        class_preds_valid,
                        class_probs_valid,
                        f"Pathology: {class_name}"
                    )

                    pathology_curve_data[class_name] = {
                        'targets': class_targets_valid,
                        'probabilities': class_probs_valid
                    }
        ...
        return detailed_results, tb_metrics, pathology_metrics, pathology_curve_data
    ...
    def _calculate_binary_metrics(self, targets, predictions, probabilities, name=""):
        ...
        try:
            ...
            logger.info(
                f"\n{name} - Probability stats - min: {df['probability'].min():.4f}, "
                f"max: {df['probability'].max():.4f}, mean: {df['probability'].mean():.4f}"
            )
...
# Historical example:
# python3 -m ultrai.training.train --dataset benin --config configs/cscs/train.yaml
